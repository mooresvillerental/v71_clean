...
# Anchor on the existing no-trend-filter arg (confirmed by usage output)
m = re.search(r'(?m)^(?P<indent>[ \t]*)parser\.add_argument\(\s*"--no-trend-filter"[^\n]*\)\s*$', src)
if not m:
    raise SystemExit("[FAIL] Could not find parser.add_argument(\"--no-trend-filter\" ...) anchor")
...
# Insert immediately after the --no-trend-filter line
insert_at = m.end()
src = src[:insert_at] + block + src[insert_at:]

p.write_text(src, encoding="utf-8")
print("[OK] Added regime CLI args after --no-trend-filter")
