...
def _mirror_v69_state_json(engine_state: dict, action: str, reason: str) -> None:
    """
    Mirror latest decision into ~/v69_app_data/state.json so app/watch_signals.py can speak/notify.
    Schema must match app/state_reader.py expectations.
    """
    try:
        home = _Path.home()
        outp = home / "v69_app_data" / "state.json"
        ...
        obj = {
            "ts": engine_state.get("ts") or int(_time.time()),
            "decision": {"action": action, "reason": reason},
            "primary": {"symbol": symbol, "price": price},
        }

        tmp = outp.with_suffix(".json.tmp")
        tmp.write_text(_json.dumps(obj, indent=2, sort_keys=False), encoding="utf-8")
        tmp.replace(outp)
    except Exception:
        # Never let mirroring break the server.
        return
...
def _build_portfolio_payload(settings: dict) -> dict:
    ...
    btc_qty = float(holdings.get("BTC-USD") or 0.0)
    current_price = float(prices.get("BTC-USD") or 0.0)
    avg_price = float(st.get("avg_price_usd") or st.get("pos_entry_price") or 0.0)
    unrealized_pl = ((current_price - avg_price) * btc_qty) if (btc_qty and current_price and avg_price) else 0.0

    trailing_stop_price = None
    high_water_price = float(st.get("high_water_price") or 0.0)

    if btc_qty > 0 and current_price > 0:
        if high_water_price <= 0:
            high_water_price = current_price
        else:
            high_water_price = max(high_water_price, current_price)

        initial_stop = (avg_price * 0.97) if avg_price > 0 else (current_price * 0.97)
        trailing_stop = high_water_price * 0.97
        trailing_stop_price = round(max(initial_stop, trailing_stop), 2)

        st["high_water_price"] = round(high_water_price, 2)
        try:
            _write_json(SETTINGS_PATH, st)
        except Exception:
            pass
    else:
        if st.get("high_water_price"):
            st["high_water_price"] = 0.0
            try:
                _write_json(SETTINGS_PATH, st)
            except Exception:
                pass
...
def _ez_build_signals_from_candles(candles):
    ...
    # A+B confirmation:
    #   - ARM when RSI enters extreme zone (<=30 oversold, >=70 overbought)
    #   - FIRE only when RSI turns back AND price confirms (green/red momentum)
    arr = []
    for c in (candles or []):
        try:
    ...
    signals = []
    armed_buy = False   # oversold armed
    armed_sell = False  # overbought armed
    ...
    for i in range(len(arr)):
        ...
        if prev_r is not None and prev_close is not None:
            # Momentum "candle color" proxy using close-to-close
            is_green = cl > prev_close
            is_red = cl < prev_close

            # BUY confirmation: RSI turns up + green close after being oversold
            if armed_buy and (r > prev_r) and is_green:
                signals.append({"time": int(arr[i]["time"]), "side": "BUY", "price": cl, "rsi": float(r)})
                armed_buy = False  # reset so we don't spam buys until re-armed

            # SELL confirmation: RSI turns down + red close after being overbought
            elif armed_sell and (r < prev_r) and is_red:
                signals.append({"time": int(arr[i]["time"]), "side": "SELL", "price": cl, "rsi": float(r)})
                armed_sell = False  # reset so we don't spam sells until re-armed
...
def latest_signal_from_engine(engine_state: dict) -> dict:
    ...
    _mirror_v69_state_json(engine_state, action, reason)
    ...
    return {
        ...
        "regime": "-",
        "trend": "-",
        "trade_eligible": action in ("BUY", "SELL"),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        from app.paths import APP_STATE_PATH as _APP_STATE_PATH
        appst  = _read_json(_APP_STATE_PATH)
        ...
        if path == "/signal":
            try:
                ...
                import json as _json
                with _urlreq.urlopen("http://127.0.0.1:8000/latest-signal", timeout=3) as _resp:
                    _live = _json.loads(_resp.read().decode("utf-8"))
                if isinstance(_live, dict) and _live:
                    sig = {
                        "action": _live.get("final_action") or _live.get("action") or "HOLD",
                        "reason": _live.get("eligibility_reason") or _live.get("quality_reason") or "OK",
                        "symbol": _live.get("symbol") or "BTC-USD",
                        "price": _live.get("price"),
                        "ts": _live.get("timestamp"),
                        "engine_price": _live.get("live_price") if _live.get("live_price") is not None else _live.get("price"),
                        "rsi": _live.get("rsi"),
                        "confidence": _live.get("confidence"),
                        "strategy": _live.get("strategy"),
                        "regime": _live.get("regime"),
                        "trend": _live.get("trend"),
                        "trade_eligible": _live.get("trade_eligible"),
                        "quality_blocked": _live.get("quality_blocked"),
                    }
                    try:
                        from strategy_weighting import adjust_confidence
                        sig["confidence"] = adjust_confidence(
                            sig.get("strategy"),
                            sig.get("confidence")
                        )
                    except Exception:
                    ...
                    try:
                        _strat = str(sig.get("strategy") or "").upper().strip()
                        _conf = float(sig.get("confidence") or 0)

                        if _strat in {"", "NONE", "UNKNOWN", "NO_SIGNAL"}:
                            sig["trade_eligible"] = False
                            sig["reason"] = "no_valid_strategy"
                        elif _conf < 35:
                            sig["trade_eligible"] = False
                            sig["reason"] = "low_confidence"
                    except Exception:
                        pass

                    try:
                        eng2 = _read_json(ENGINE_STATE_PATH) or {}
                        eng2["signal_meta"] = {
                        ...
                        _write_json(ENGINE_STATE_PATH, eng2)
                    except Exception:
                        pass
            ...
            # --- BALANCE-BASED POSITION SIZING (USD display for BUY and SELL) ---
            try:
                conf = float(sig.get("confidence") or 0.0)
                cash_usd = float(engine.get("cash_usd") or 0.0)

                holdings = engine.get("holdings") or {}
                btc_qty = float(holdings.get(sig.get("symbol") or "BTC-USD") or 0.0)
                px = float(sig.get("price") or 0.0)
                holdings_usd = btc_qty * px

                # confidence tiers -> percent of available balance
                if conf >= 70:
                    pct = 0.20
                elif conf >= 50:
                    pct = 0.15
                ...
                if sig.get("action") == "BUY":
                    usd_amt = cash_usd * pct
                    usd_amt = round(usd_amt)
                    sig["suggested_trade_usd"] = max(0, usd_amt)
                    sig["suggested_trade_qty"] = round((usd_amt / px), 8) if px > 0 else 0.0

                elif sig.get("action") == "SELL":
                    usd_amt = holdings_usd * pct
                    usd_amt = round(usd_amt)
                    sig["suggested_trade_usd"] = max(0, usd_amt)
                    sig["suggested_trade_qty"] = round((usd_amt / px), 8) if px > 0 else 0.0

                else:
                    sig["suggested_trade_usd"] = 0
                    sig["suggested_trade_qty"] = 0.0

                # simple protective stop
                if px > 0:
                    if sig.get("action") == "BUY":
                        sig["stop_loss_price"] = round(px * 0.97, 2)
                    elif sig.get("action") == "SELL":
                        sig["stop_loss_price"] = round(px * 1.03, 2)
                    else:
                        sig["stop_loss_price"] = None
                else:
                    sig["stop_loss_price"] = None
            except Exception:
                pass
            ...
            try:
                ...
                vol_boost = min(10, distance * 0.2)
                trend_boost = 10 if distance > 10 else 0

                conf = distance * 1.5 + vol_boost + trend_boost

                sig["confidence"] = int(min(100, max(0, conf)))
                sig["strategy"] = "RSI_MEAN_REVERSION"
                sig["regime"] = engine.get("mode", "-")
                sig["trend"] = "UP" if rsi > 50 else "DOWN"
                sig["trade_eligible"] = sig["action"] in ("BUY","SELL")
                sig["quality_blocked"] = False
            except Exception:
                pass
            ...
            try:
                ...
                # simulate basic intelligence from engine data
                rsi = primary.get("rsi") or 50
                sig["strategy"] = "RSI_MEAN_REVERSION"
                sig["regime"] = engine.get("mode", "-")
                sig["trend"] = "UP" if (primary.get("rsi") or 50) > 50 else "DOWN"
                sig["trade_eligible"] = sig["action"] in ("BUY","SELL")
                sig["quality_blocked"] = False
            except Exception:
                pass
        ...
        if path == "/trade-history":
            try:
                fp = os.path.join(os.path.expanduser("~"), "v71_clean", "signals", "trade_history.jsonl")
                rows = []
                    ...
                    with open(fp, "r", encoding="utf-8") as f:
                        for line in f:
                            try:
                                ...
                                pass
                return self._send_json(200, {"ok": True, "trades": rows[-50:]})
            except Exception as e:
        ...
        if path == "/opportunities":
            ...
            for sym in syms:
                ...
                try:
                    ...
                    # --- sizing / holdings (read-only) ---
                    _hold_qty = 0.0
                    try:
                        from app.paths import APP_STATE_PATH
                        from pathlib import Path as _P
                        import json as _json
                        _stp = _P(APP_STATE_PATH)
                        _state = _json.loads(_stp.read_text("utf-8")) if _stp.exists() else {}
                        _h = _state.get("holdings") or {}
                        _hold_qty = float(_h.get(sym) or 0.0)
                    except Exception:
                        _hold_qty = 0.0
                    ...
                    _rec_qty = None
                    try:
                        _pool = float((_st.get("tactical_pool_usd") if isinstance(_st, dict) else None) or 0.0)
                    except Exception:
                        _pool = 0.0

                    # Recommendation rules (Phase 2):
                    # BUY  -> use tactical pool USD (qty = usd/price)
                    # SELL -> sell current holdings only (prevents fake sells)
                    if action == "BUY" and _pool > 0:
                        _rec_usd = _pool
                        try:
                            _rec_qty = float(_rec_usd) / float(last)
                        except Exception:
                            _rec_qty = None
                    elif action == "SELL":
                        if _hold_qty and _hold_qty > 0:
                            _rec_qty = float(_hold_qty)
                            _rec_usd = float(_hold_qty) * float(last)

                    # --- confidence (0-100) + AI overlay (read-only) ---
                    base_conf = 0
                    try:
                        if action in ('BUY','SELL'):
                            base_conf = int(max(0, min(100, round(float(score) * 6.0))))
                    except Exception:
                        base_conf = 0
                    
                    features = {}
                    try:
                        win = closes[-30:] if (isinstance(closes, list) and len(closes) >= 30) else list(closes or [])
                        if win and len(win) >= 2:
                            base0 = float(win[0])
                            lastf = float(win[-1])
                            if base0:
                                features['trend_slope_pct'] = (lastf - base0) / base0 * 100.0
                            hi = max(win)
                            lo = min(win)
                            if lastf:
                                features['vol_pct'] = (float(hi) - float(lo)) / float(lastf) * 100.0
                            # simple peak-risk proxy (placeholder for your peak watchers)
                            if action == 'SELL' and hi:
                                features['peak_risk'] = (float(lastf) / float(hi)) * 100.0
                            elif action == 'BUY' and lastf:
                                features['peak_risk'] = (float(lo) / float(lastf)) * 100.0
                    except Exception:
                        features = {}
                    
                    ai_conf = base_conf
                    ai_delta = 0
                    ai_expl = 'AI overlay inactive.'
                    try:
                        from app.ai_advisor import ai_adjust_confidence
                        adv = ai_adjust_confidence(symbol=sym, action=action, base_confidence=base_conf, features=features)
                        ai_conf = int(getattr(adv, 'ai_confidence', base_conf))
                        ai_delta = int(getattr(adv, 'delta', 0))
                        ai_expl = str(getattr(adv, 'explanation', '')).strip() or ai_expl
                    except Exception:
                        pass
                    ...
                    o = {
                        ...
                        'base_confidence': int(base_conf),
                        'ai_confidence': int(ai_conf),
                        'ai_delta': int(ai_delta),
                    ...
                    if (prev is None) or (float(o.get("score") or 0.0) > float(prev.get("score") or 0.0)):
                        ...
                        try:
                            eng = _read_json(ENGINE_STATE_PATH) or {}
                            eng["signal_meta"] = {
                                "symbol": o.get("symbol"),
                                "action": o.get("action"),
                                "price": o.get("price"),
                                ...
                                "regime": eng.get("mode", "-"),
                                "trend": (features.get("trend_slope_pct") if 'features' in locals() else None),
                                "trade_eligible": o.get("action") in ("BUY","SELL"),
                                "quality_blocked": False,
                                "recommended_usd": o.get("recommended_usd"),
                                "recommended_qty": o.get("recommended_qty"),
                                "ai_delta": o.get("ai_delta"),
                                "ts": int(time.time())
                            }
                            _write_json(ENGINE_STATE_PATH, eng)
                        except Exception:
                            pass
                except Exception:
                    continue
        ...
        if path == "/shadow-stats":
            try:
                import json as _json
                from pathlib import Path as _Path

                def _read_jsonl_count(fp):
                    p = _Path(fp)
                    if not p.exists():
                        return 0, []
                    rows = []
                ...
                closed_n, closed_rows = _read_jsonl_count("signals/shadow_trades_closed.jsonl")
                open_n, open_rows = _read_jsonl_count("signals/shadow_trades_open.jsonl")
                blocked_n, blocked_rows = _read_jsonl_count("signals/shadow_trades_blocked.jsonl")
                ...
                for r in closed_rows:
                    if str(r.get("outcome", "")).upper() == "WIN":
                        wins += 1
                    try:
                        pnl_vals.append(float(r.get("pnl_pct", 0)))
                    except Exception:
                        pass

                losses = max(0, closed_n - wins)
                win_rate = round((wins / closed_n) * 100, 2) if closed_n else 0.0
                avg_pl = round(sum(pnl_vals) / len(pnl_vals), 4) if pnl_vals else 0.0
                ...
                return self._send_json(500, {"ok": False, "error": "shadow_stats_failed", "detail": str(e)})

        if path == "/shadow-last-closed":
            try:
                import json as _json
                from pathlib import Path as _Path
                fp = _Path("signals/shadow_trades_closed.jsonl")
                last_row = None
                if fp.exists():
                    with fp.open("r", encoding="utf-8", errors="ignore") as f:
                        for line in f:
                            ...
                            if not line:
                                continue
                            try:
                                last_row = _json.loads(line)
                            except Exception:
                                pass
                return self._send_json(200, {"ok": True, "trade": last_row})
            except Exception as e:
                return self._send_json(500, {"ok": False, "error": "shadow_last_closed_failed", "detail": str(e)})

        if path == "/strategy-stats":
            try:
                import json as _json
                from pathlib import Path as _Path
                fp = _Path("logs/ezcore_v1_signal_analytics.json")
                if not fp.exists():
                    return self._send_json(200, {"ok": True, "strategies": []})

                data = _json.loads(fp.read_text("utf-8"))
                rows = []
                for strat_name, strat_payload in (data or {}).items():
                    for side_name, side_payload in (strat_payload or {}).items():
                        for timeframe, vals in (side_payload or {}).items():
                            rows.append({
                                "strategy": strat_name,
                                "side": side_name,
                                "timeframe": timeframe,
                                "samples": vals.get("n", 0),
                                "wins": vals.get("wins", 0),
                                "win_pct": vals.get("win_pct", 0),
                                "avg_move_pct": vals.get("avg_move_pct", 0),
                                "avg_abs_move_pct": vals.get("avg_abs_move_pct", 0),
                            })

                rows.sort(key=lambda x: (-float(x.get("win_pct", 0) or 0), -int(x.get("samples", 0) or 0)))
                return self._send_json(200, {"ok": True, "strategies": rows})
            except Exception as e:
                return self._send_json(500, {"ok": False, "error": "strategy_stats_failed", "detail": str(e)})
