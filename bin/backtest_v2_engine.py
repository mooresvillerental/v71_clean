...
def _regime_mult_for_time(t, *, enabled, slope_thr, chop_thr, trend_mult, chop_mult):
    """V2 scaffold: returns a sizing multiplier based on market regime.
    Currently stubbed to 1.0 (no behavior change). We'll implement regime logic next.
    """
    if not enabled:
        return 1.0
    # TODO(v2): compute regime from daily close vs EMA and slope/chop.
    # Keep stub 1.0 so v2 compiles and runs identically to v1 until we implement.
    return 1.0
...
def main():
    ...
    _p.add_argument("--trade-pct-dyn", action="store_true", help="Enable dynamic trade_pct based on drawdown (default off)")
    _p.add_argument("--trade-pct-hi", type=float, default=None, help="trade_pct when DD < switch (default: --trade-pct)")
    _p.add_argument("--trade-pct-lo", type=float, default=0.30, help="trade_pct when DD >= switch (default 0.30)")
    _p.add_argument("--trade-pct-switch-dd", type=float, default=None, help="DD%% threshold to switch hi->lo (default: --soft1-pct)")
    ...
    _p.add_argument("--regime-mode", action="store_true", help="Enable regime sizing multiplier (default off)")
    _p.add_argument("--regime-symbol", type=str, default="BTC-USD", help="Regime reference symbol (default BTC-USD)")
    _p.add_argument("--regime-ema-len", type=int, default=200, help="EMA length used for regime (default 200)")
    _p.add_argument("--regime-slope-thr", type=float, default=0.0, help="Slope threshold for trend vs flat (default 0.0)")
    _p.add_argument("--regime-chop-thr", type=float, default=0.0, help="Chop threshold (reserved, default 0.0)")
    _p.add_argument("--regime-trend-mult", type=float, default=1.15, help="Multiplier when regime is favorable/trending (default 1.15)")
    _p.add_argument("--regime-chop-mult", type=float, default=0.85, help="Multiplier when regime is choppy/unfavorable (default 0.85)")
    _p.add_argument("--rsi-turnup", action="store_true", help="Require RSI to be turning up before BUY (default off)")
    _p.add_argument("--rsi-turnup-bars", type=int, default=1, help="Consecutive RSI increases required before BUY (default 1)")
    ...
    regime_mode = bool(getattr(_a, "regime_mode", False))
    regime_symbol = str(getattr(_a, "regime_symbol", "BTC-USD")).strip().upper() or "BTC-USD"
    regime_ema_len = int(getattr(_a, "regime_ema_len", 200) or 200)
    regime_slope_thr = float(getattr(_a, "regime_slope_thr", 0.0) or 0.0)
    regime_chop_thr = float(getattr(_a, "regime_chop_thr", 0.0) or 0.0)
    regime_trend_mult = float(getattr(_a, "regime_trend_mult", 1.15) or 1.15)
    regime_chop_mult = float(getattr(_a, "regime_chop_mult", 0.85) or 0.85)

    rsi_turnup = bool(getattr(_a, "rsi_turnup", False))
    rsi_turnup_bars = max(1, int(getattr(_a, "rsi_turnup_bars", 1)))
    ...
    trade_pct_dyn = bool(getattr(_a, 'trade_pct_dyn', False))
    trade_pct_hi = getattr(_a, 'trade_pct_hi', None)
    trade_pct_lo = float(getattr(_a, 'trade_pct_lo', 0.30))
    trade_pct_switch_dd = getattr(_a, 'trade_pct_switch_dd', None)
    ...
    for (t, sym, side, price, rsi, conf) in events:
        ...
        if pos_sym is None:
            ...
            if regime_mode:
                try:
                    regime_mult = float(_regime_mult_for_time(t, enabled=True, slope_thr=regime_slope_thr, chop_thr=regime_chop_thr, trend_mult=regime_trend_mult, chop_mult=regime_chop_mult))
                except Exception:
                    regime_mult = 1.0
            
            # FIX: ensure trade_pct_eff exists in older backup paths
            trade_pct_eff = trade_pct
            usd = max(MIN_TRADE_USD, cash*trade_pct_eff*mult*vol_mult*regime_mult)
    ...
    trades_csv = out_dir / f"trades_{days}d_{stamp}.csv"
    equity_csv = out_dir / f"equity_{days}d_{stamp}.csv"
