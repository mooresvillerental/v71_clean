...
# CACHE_CANDLES_V1 helpers
import os as _os, json as _json, gzip as _gzip, hashlib as _hashlib
...
def _cache_path(cache_dir, key):
    return str(Path(cache_dir) / f"candles_{key}.json.gz")
...
def _fetch_with_cache(fetch_fn, *, cache_dir, use_cache, cache_parts, fetch_kwargs):
    """
    cache_parts should uniquely describe the request, e.g. (src, sym, interval, since, now).
    fetch_kwargs will be passed to fetch_fn(**fetch_kwargs).
    """
    if not use_cache:
        return fetch_fn(**fetch_kwargs)

    _cache_mkdir(cache_dir)
    key = _cache_key(*cache_parts)
    path = _cache_path(cache_dir, key)
    hit = _cache_load(path)
    if hit is not None:
        # Expected schema: {"candles": [...]}
        c = hit.get("candles")
        if isinstance(c, list) and c:
            print(f"DEBUG cache HIT: {path}")
            return c

    candles = _fetch_with_cache(
        fetch_fn,
        cache_dir=cache_dir,
        use_cache=use_cache,
        cache_parts=(src, sym, interval, since, now),
        fetch_kwargs=dict(**fetch_kwargs),
    )
    if isinstance(candles, list) and candles:
        _cache_save(path, {"candles": candles})
        print(f"DEBUG cache MISS->SAVE: {path}")
    return candles
...
def main():
    ...
    # CACHE_CANDLES_V1 (default-on): cache fetched candles to disk to speed up repeat tests
    _p.add_argument("--cache-dir", type=str, default="data_cache", help="Directory for candle cache (default data_cache)")
    _p.add_argument("--no-cache", action="store_true", help="Disable candle cache (always refetch)")
    ...
    cache_dir = str(getattr(_a, 'cache_dir', 'data_cache') or 'data_cache')

    use_cache = (not bool(getattr(_a, 'no_cache', False)))
    # WR_GATE_V1
