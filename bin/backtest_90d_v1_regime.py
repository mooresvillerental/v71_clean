...
def main():
    ...
    # REGIME_MODE_V1 (default-off): daily regime multiplier using 1D close vs EMA + EMA slope
    _p.add_argument("--regime-mode", action="store_true", help="Enable daily regime sizing (default off)")
    _p.add_argument("--regime-symbol", type=str, default="BTC-USD", help="Symbol used for regime calc (default BTC-USD)")
    _p.add_argument("--regime-ema-len", type=int, default=200, help="EMA length for regime (default 200)")
    _p.add_argument("--regime-slope-pct", type=float, default=0.05, help="Min EMA slope %% per day for risk-on (default 0.05)")
    _p.add_argument("--regime-chop-pct", type=float, default=1.00, help="Max |close-ema| %% for risk-on (default 1.00)")
    _p.add_argument("--regime-riskon-mult", type=float, default=1.00, help="Sizing multiplier when risk-on (default 1.00)")
    _p.add_argument("--regime-riskoff-mult", type=float, default=0.60, help="Sizing multiplier when risk-off (default 0.60)")
    _a = _p.parse_args()
    ...
    regime_slope_pct = float(getattr(_a, "regime_slope_pct", 0.05) or 0.05)
    regime_chop_pct = float(getattr(_a, "regime_chop_pct", 1.00) or 1.00)
    regime_riskon_mult = float(getattr(_a, "regime_riskon_mult", 1.00) or 1.00)
    regime_riskoff_mult = float(getattr(_a, "regime_riskoff_mult", 0.60) or 0.60)
    ...
    # REGIME_MODE_V1_BUILD: build dict day_bucket -> (regime, mult, close, ema, ema_prev)
    _regime_by_day = None  # dict int(day_bucket)-> tuple
    def _ema_series_regime(vals, n):
        if not vals or n <= 1:
            return list(vals)
        k = 2.0 / (n + 1.0)
        out = []
    ...
    def _build_regime_by_day():
        nonlocal _regime_by_day
        if not regime_mode:
            _regime_by_day = None
            return
        d = data.get(regime_symbol)
        if not d:
            _regime_by_day = None
            return
        ...
        # last close per UTC day bucket
        last_close = {}
        for t, c in zip(times, closes):
            try:
                day = int(t) // 86400
                last_close[day] = float(c)
            except Exception:
                continue
        if not last_close:
            _regime_by_day = None
        ...
        days = sorted(last_close.keys())
        day_closes = [last_close[d] for d in days]
        emas = _ema_series_regime(day_closes, int(regime_ema_len))

        out = {}
        for i, day in enumerate(days):
            close = float(day_closes[i])
            ema = float(emas[i]) if i < len(emas) else float(day_closes[i])
            ema_prev = float(emas[i-1]) if i > 0 and (i-1) < len(emas) else None

            # slope % per day (ema vs ema_prev)
            slope_pct = 0.0
            if ema_prev and ema_prev != 0.0:
                slope_pct = ((ema - ema_prev) / ema_prev) * 100.0

            # chop: distance of close from ema (%)
            chop_pct = 0.0
            if ema != 0.0:
                chop_pct = abs(close - ema) / ema * 100.0

            risk_on = (slope_pct >= float(regime_slope_pct)) and (chop_pct <= float(regime_chop_pct))
            regime = "RISK_ON" if risk_on else "RISK_OFF"
            mult = float(regime_riskon_mult) if risk_on else float(regime_riskoff_mult)

            out[int(day)] = (regime, mult, close, ema, ema_prev)
        _regime_by_day = out

    _build_regime_by_day()
        ...
        if pos_sym is None:
            ...
            if vol_mode:
                try:
                    ...
                    vol_mult = 1.0
            regime_mult = 1.0
            if regime_mode and _regime_by_day is not None:
                try:
                _day = int(t) // 86400
            regime_mult = float((_regime_by_day.get(_day) or (None, 1.0))[1])
            except Exception:
                regime_mult = 1.0
            usd = max(MIN_TRADE_USD, cash*trade_pct*mult*vol_mult*regime_mult)
