...
if not anchor:
    print("FIX FAILED: could not find chart anchor comment")
    sys.exit(1)

# If a broken opportunities block exists, remove everything from its marker up to the chart anchor
m = re.search(r'^\s*#\s*' + re.escape(MARK) + r'\s*$', s, flags=re.M)
if m:
    s = s[:m.start()] + s[anchor.start():]
    # re-find anchor after deletion
    anchor = re.search(r'^\s*# --- Chart endpoint: live price \(read-only\) ---\s*$', s, flags=re.M)
    if not anchor:
        print("FIX FAILED: anchor missing after removal")
        sys.exit(1)
...
if not m_if:
    print('FIX FAILED: could not find line: if path == "/price":')
    sys.exit(1)

IND = m_if.group("ind")  # exact indentation used by other routes

# Build a clean opportunities route using the same IND
block = f'''
{IND}# {MARK}
...
{IND}        except Exception:
{IND}            syms = ["BTC-USD","ETH-USD","SOL-USD","DOGE-USD"]
...
print("OK ✅ opportunities block rewritten with correct indentation")
